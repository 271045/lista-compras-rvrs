...
with st.sidebar:
    ...
    if arquivos_salvos:
        ...
        if lista_escolhida != "Selecionar...":
            ...
            with c2:
                key_confirma = f"confirma_excluir_{lista_escolhida}"
                if key_confirma not in st.session_state:
                    st.session_state[key_confirma] = False

                if not st.session_state[key_confirma]:
                    if st.button("🗑️ EXCLUIR", use_container_width=True):
                        st.session_state[key_confirma] = True
                        st.rerun()
                else:
...
with st.sidebar:
    ...
    if itens_para_exportar:
        ...
        st.download_button("🖼️ BAIXAR IMAGEM", img_bytes, f"lista.png", "image/png", use_container_width=True)
